...
import { normalizeVideoDetail } from "@/lib/platforms/youtube/youtube-normalize";
...
export async function POST(request: Request) {
  try {
    ...
    const response = await getYouTubeClient().videosList<{ items?: Array<Parameters<typeof normalizeVideoDetail>[0]> }>({
      part: "snippet,contentDetails,statistics,status,liveStreamingDetails",
      id: ids.join(","),
    });

    return Response.json({
      items: (response.items ?? []).map(normalizeVideoDetail),
      quotaCostEstimate: estimateQuotaCost("videosList"),
    });
  } catch (error) {
