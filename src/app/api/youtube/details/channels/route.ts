...
import { normalizeChannelDetail } from "@/lib/platforms/youtube/youtube-normalize";
...
export async function POST(request: Request) {
  try {
    ...
    const response = await getYouTubeClient().channelsList<{ items?: Array<Parameters<typeof normalizeChannelDetail>[0]> }>({
      part: "snippet,contentDetails,statistics",
      id: ids.join(","),
    });

    return Response.json({
      items: (response.items ?? []).map(normalizeChannelDetail),
